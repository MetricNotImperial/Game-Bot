...
def checkBalls():
    ...
    if pyautogui.locateOnScreen('OutOfBalls.png') != None:
        print("da ring ding ting showed up so balls at 0")
        time.sleep(0.2)
        ...
        for i in range(8):
            time.sleep(np.random.uniform(0.1,0.5))
            pyautogui.keyDown("q")
            time.sleep(np.random.uniform(0.25,0.40))
            pyautogui.keyUp("q")

        #if the the 'q' for dialogue earlier was to fast and the last one was still there
        if pyautogui.locateOnScreen('TooFast.png') != None:
            time.sleep(np.random.uniform(0.1,0.5))
            pyautogui.keyDown("q")
            time.sleep(np.random.uniform(0.25,0.40))
            pyautogui.keyUp("q")
